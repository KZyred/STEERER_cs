...
class HighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
                 num_channels, fuse_method, multi_scale_output=True,module_idx=None,stage_idx = None):
        ...
        if self.stage_idx==4 and self.module_idx== 2:
    ...
    def _make_fuse_layers(self):
        ...
        for i in range(num_branches if self.multi_scale_output else 1):
            ...
            for j in range(num_branches):
                if j > i:
                    ...
                    # factor =min(i+2, num_branches) - max(i-1,0)
                    fuse_layer.append(nn.Sequential(
                        nn.Conv2d(num_inchannels[i]*num_branches,
                                  num_inchannels[i],
                                  1,
                                  1,
                                  0,
        ...
        if self.stage_idx==4 and self.module_idx==2:
            return nn.ModuleList(fuse_layers)  #fuse_layers[:-2]
        else:
    ...
    def forward(self, x):
        ...
        for i in range(len(self.fuse_layers)):
            ...
            for j in range(1, self.num_branches):
                if i == j:
                    ...
                    y.append(F.interpolate(
                        self.fuse_layers[i][j](x[j]),
                        size=[height_output, width_output],
                        mode='bilinear'))
                else:
                    y.append(self.fuse_layers[i][j](x[j]))
            y = torch.cat(y,1)
            x_fuse.append(self.fuse_layers[i][i](y))
        ...
        # for i in range(len(self.fuse_layers)):
            ...
            # y = [x[i]]
            # if i-1>=0:
            #     y.append(self.fuse_layers[i][i-1](x[i-1]))
            # if i+1<self.num_branches:
            #     y.append(F.interpolate(self.fuse_layers[i][i+1](x[i+1]),
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        #     nn.ReLU()
        # )
        # self.down_stage4 = nn.Sequential(
        # nn.Conv2d(pre_stage_channels[3], 48, kernel_size=1, stride=1, padding=0, bias=False),
        # nn.BatchNorm2d(48, momentum=BN_MOMENTUM),
        # nn.ReLU(inplace=True))
        #
        # self.down_stage3 = nn.Sequential(
        # nn.Conv2d(pre_stage_channels[2], 48, kernel_size=1, stride=1, padding=0, bias=False),
        # nn.BatchNorm2d(48, momentum=BN_MOMENTUM),
        # nn.ReLU(inplace=True))
        #
        # self.down_stage2 = nn.Sequential(
        # nn.Conv2d(pre_stage_channels[1], 48, kernel_size=1, stride=1, padding=0, bias=False),
        # nn.BatchNorm2d(48, momentum=BN_MOMENTUM),
        # nn.ReLU(inplace=True))
    ...
    def _make_stage(self, layer_config, num_inchannels,
                    multi_scale_output=True, stage_idx=None):
