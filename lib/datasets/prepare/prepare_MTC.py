...
mode = 'train'
Root_ori = '/mnt/petrelfs/hantao/ProcessedData/Maize Tassel Counting Dataset'
Root_dst = '/mnt/petrelfs/hantao/ProcessedData/MTC'
train_path =  os.path.join(Root_dst)
test_path =  os.path.join(Root_dst)

train_set = ['Zhengzhou2012', 'Zhengzhou2010','Taian2010_1', 'Taian2011_1', 'Taian2012_1', 'Taian2013_1', 'Gucheng2012', 'Jalaid2015_1']
test_set = ['Zhengzhou2011', 'Taian2010_2', 'Taian2011_2', 'Taian2012_2', 'Taian2013_2', 'Gucheng2014', 'Jalaid2015_2', 'Jalaid2015_2']
test_txt = os.path.join(Root_dst, 'image_sets', 'test.txt')
train_list = []
for scene in train_set:
    train_list +=  glob.glob(os.path.join(Root_ori,scene,'Images','*.jpg'))



test_list = []
for scene in test_set:
    test_list+= glob.glob(os.path.join(Root_ori,scene,'Images','*.jpg'))


import  pdb
pdb.set_trace()
dst_imgs_path = os.path.join(Root_dst,'imgs')
...
def resize_images(src_path, shift=0, resize_factor = 1):
    ...
    new_test_list = ""
    shift = 0
    for set in [train_list,test_list]:
        for idx, img_path in enumerate(set):
            ...
            gt_path = img_path.replace('.jpg','.mat').replace('Images', 'Annotations')

            if img_path in train_list:
                new_train_list+= (img_id+'\n')
            elif img_path in test_list:
            ...
            # assert  img_name.split('.')[0] in mat_list[idx]
            ...
            annPoints = gtInf['annotation'][0][0][1]
