...
mode = 'train'
Root = '/mnt/petrelfs/hantao/ProcessedData/TRANCOS_v3'
train_path =  os.path.join(Root)
test_path =  os.path.join(Root)

train_txt = os.path.join(Root, 'image_sets', 'trainval.txt')
test_txt = os.path.join(Root, 'image_sets', 'test.txt')
train_list = [line.strip() for line in open(train_txt)]
test_list = [line.strip() for line in open(test_txt)]

dst_imgs_path = os.path.join(Root,'imgs')
...
def resize_images(src_path, shift=0, resize_factor = 2):
    ...
    file_list.sort()
    mat_list = glob.glob(os.path.join(src_path,'images','*.mat'))
    mat_list.sort()
    print(len(file_list))
    new_train_list= ""
    new_test_list = ""
    for idx, img_path in enumerate(file_list):
        ...
        gt_path = os.path.splitext(img_path)[0]+'mask.mat'
        dot_path = os.path.splitext(img_path)[0]+'dots.png'


        img_ori = Image.open(img_path)
        gtInf = scio.loadmat( gt_path)  # format [ w, h ]
        dot_map = cv.imread(dot_path,cv.IMREAD_GRAYSCALE)

        roi = gtInf['BW']

        dot_map = dot_map*roi
        roi =  np.expand_dims(roi,2).repeat(3,axis=2)
        ...
        h_position, w_position = np.where(dot_map>0)

        annPoints = np.concatenate([w_position.reshape(-1,1), h_position.reshape(-1,1)],1)
