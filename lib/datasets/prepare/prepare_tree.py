...
import  imageio
mode = 'train'
Root_ori = '/mnt/petrelfs/hantao/ProcessedData/urban-tree-detection-data-main'
Root_dst = '/mnt/petrelfs/hantao/ProcessedData/TREE'
train_path =  os.path.join(Root_dst)
test_path =  os.path.join(Root_dst)


train_list =  [name.rstrip() for name in open(os.path.join(Root_ori,'train.txt'),'r')]
test_list = [name.rstrip() for name in open(os.path.join(Root_ori,'test.txt'),'r')]
val_list = [name.rstrip() for name in open(os.path.join(Root_ori,'val.txt'),'r')]

dst_imgs_path = os.path.join(Root_dst,'images')
dst_mask_path = os.path.join(Root_dst,'mask')
dst_json_path = os.path.join(Root_dst,'jsons')
...
def resize_images(src_path, shift=0, resize_factor = 1):

    new_train_list= ""
    new_test_list = ""
    new_val_list = ""
    shift = 0
    for set in [train_list,test_list, val_list]:

        for idx, img_name in enumerate(set):
            img_id =  int(idx) + shift
            img_id = str(img_id).zfill(4)

            img_path= os.path.join(Root_ori,'images',img_name+'.tif')
            gt_path =  csv_path = os.path.join(Root_ori,'csv',img_name + '.csv')
            ...
            if img_name in train_list:
                new_train_list+= (img_id+'\n')
            elif img_name in test_list:
                new_test_list+= (img_id+'\n')
            elif img_name in val_list:
                new_test_list+= (img_id+'\n')


            ImgInfo = {}
            ImgInfo.update({'img_id':img_id+'.jpg'})

            print(img_path, img_id)
            img_ori = imageio.imread(img_path)
            img_ori = img_ori[:,:,0:3]
            if os.path.exists(csv_path):
                annPoints = np.loadtxt(csv_path,delimiter=',',skiprows=1).astype('int')
            else:
                annPoints = np.zeros((0,2),dtype='float32')


            if len(annPoints.shape)==1:
                annPoints = annPoints[None,:] # format [ w, h ]

            # import  pdb
            # pdb.set_trace()


            w_rate, h_rate= resize_factor, resize_factor
            # import pdb
            # pdb.set_trace()
            ...
            ImgInfo.update({"points": annPoints.tolist()})

            ImgInfo.update({"human_num": len(annPoints)})

            # img_ori.save(dst_img_path, quality=95)
            cv.imwrite(dst_img_path, img_ori,[cv.IMWRITE_JPEG_QUALITY,100])
            with open(dst_json_name, 'w') as  f:
                json.dump(ImgInfo, f)
        shift += len(set)
    with open(os.path.join(Root_dst,'train.txt'), "w") as f:
        f.write(new_train_list)
    with open(os.path.join(Root_dst,'test.txt'), "w") as f:
        f.write(new_test_list)
    with open(os.path.join(Root_dst,'val.txt'), "w") as f:
        f.write(new_val_list)
...
def loc_gt_make(  mode = 'test'):
    txt_path = os.path.join(Root_dst,mode+'.txt')
    ...
    for idx, img_id in enumerate(img_ids):
        ...
        if idx != -1:
            ...
            import  pdb
            pdb.set_trace()
            for id, head in enumerate(infor['points']):
                x1, y1 = int(head[0]), int(head[1])
                ...
                r_small = 15 #int(min(w, h) / 2)
                r_large = 15 #int(np.sqrt (w * w + h * h) / 2)
            ...
            # print(Box_Info)
            with open(os.path.join(Root_dst,  mode + '_gt_loc.txt'), 'a') as f:
...
if __name__ == '__main__':
    ...
    #================1. resize images ===================
    resize_images(train_list, 0,resize_factor=1)
    # # ================2. size_map ==================
